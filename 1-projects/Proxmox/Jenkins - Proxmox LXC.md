...
- [ ] Configure GitHub plugin (webhook/credentials)
- [ ] Take initial "post-setup" snapshot before first experimental pipeline run
- [ ] If still laggy after the RAM/swap bump, consider capping the JVM heap explicitly (e.g. `JAVA_OPTS: -Xmx1536m` in `docker-compose.yml`) instead of growing the container further
